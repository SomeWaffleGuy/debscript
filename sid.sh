...
if echo "$answer" | grep -iq "^2" ;then
	...
	read answer
	if echo "$answer" | grep -iq "^y" ;then
		apt install -y nvidia-driver nvidia-cuda-dev nvidia-cuda-toolkit
